...
# Output: 4
# Explanation: We can pick up the cards [3,4,2,3] which contain a matching pair of cards with value 3. Note that picking up the cards [4,2,3,4]
# is also optimal.
# Example 2:
...
def consCard(cards):
    ...
    for i, num in enumerate(cards):
        if num in hash_map:
            ans = min(ans, i - hash_map[num] + 1)
        hash_map[num] = i

    return -1 if ans == float("inf") else int(ans)


print(consCard([3, 4, 2, 3, 4, 7]))
