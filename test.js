...
function removeProperOrder(s) {
  ...
  for (let i = 0; i < arr.length; i++) {
      const current = arr[i];
      if (stack.length >= 2 && current === stack[stack.length - 1] && current === stack[stack.length - 2]) {
          stack.pop();
          stack.pop();
      } else {
          stack.push(current);
...
// Example usage:
const input = 'abababac';
const output = removeProperOrder(input);
console.log(output); // Output will be 'abc'